...
namespace com { namespace base
{
		...
		void **ppv) 
	{
		...
		if (FAILED(hr))
		{
			...
			pNewComponent->NDRelease();
			return hr;
		}

		// Get the requested interface.
		hr = pNewComponent->NDQueryInterface(iid, ppv);
	...
	{
		com::Trace(L"[%s] : active components #%d, server locks #%d\n", 
			__FUNCTIONW__, CUnknown::ActiveComponents(), s_cServerLocks);
	...
	BOOL CFactory::StartFactories()
	{
		...
		for(CFactoryData *pData = pStart; pData <= pEnd; pData++)
		{
			...
			pData->m_pIClassFactory = nullptr;
			pData->m_dwRegister = 0;
